...
class Net(nn.Module):
  ...
  def forward(self, x):
    ...
    x = F.max_pool2d(self.sig(self.batch1(self.conv1(x))), 2)
    # Conv(64, 128, 3, 1, 1) - BatchNorm(128) - ReLU - MaxPool(2, 2)
    x = F.max_pool2d(self.sig(self.batch2(self.conv2(x))), 2)
    # Conv(128, 256, 3, 1, 1) - BatchNorm(256) - ReLU
    x = self.sig(self.batch3(self.conv3(x)))
    # Conv(256, 256, 3, 1, 1) - BatchNorm(256) - ReLU - MaxPool(2, 2)
    x = F.max_pool2d(self.sig(self.batch3(self.conv4(x))), 2)
    # Conv(256, 512, 3, 1, 1) - BatchNorm(512) - ReLU
    x = self.sig(self.batch4(self.conv5(x)))
    # Conv(512, 512, 3, 1, 1) - BatchNorm(512) - ReLU - MaxPool(2, 2)
    x = F.max_pool2d(self.sig(self.batch4(self.conv6(x))), 2)
    # Conv(512, 512, 3, 1, 1) - BatchNorm(512) - ReLU
    x = self.sig(self.batch4(self.conv6(x)))
    # Conv(512, 512, 3, 1, 1) - BatchNorm(512) - ReLU - MaxPool(2, 2)
    x = F.max_pool2d(self.sig(self.batch4(self.conv6(x))), 2)
    x = torch.flatten(x, 1)
    # Linear(512, 4096) - ReLU - Dropout(0.5)
    x = self.dropout(self.sig(self.fc1(x)))
    # Linear(4096, 10) - ReLU - Dropout(0.5)
    x = self.dropout(self.sig(self.fc2(x)))
    # Linear(4096, 10)
    x = self.fc3(x)
    return x
